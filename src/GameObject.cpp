...
GameObject::GameObject(glm::vec3 position, std::shared_ptr<TTK::MeshBase> _mesh, std::shared_ptr<Material> _material)
	: m_pScale(1.0f),
...
void GameObject::draw(TTK::Camera &camera)
{
	...
	material->mat4Uniforms["u_model"] = m_pLocalToWorldMatrix;
